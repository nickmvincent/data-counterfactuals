---
title: Discussions and project links
description: Ongoing discussion venues and adjacent project links for the data counterfactuals project.
lede: A lightweight directory of places to follow the project, browse related work, and jump into discussion.
notes:
  header:
    label: What this page is
    body:
      - This page pulls from the shared project network metadata, so it can list discussion venues and nearby projects in one place.
      - Think of it as a directory for following along, not as a standalone forum hosted on this site.
---
